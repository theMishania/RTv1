...
int cast_ray(t_cl_scene *cl_scene, double3 start, double3 dir, char flag)
{
	t_cl_obj *ptr = 0;
	double closest_t;
	double intensity;
	...
	while (i >= 0)
	{
		...
		if ((ptr->rgb.r *= intensity) >= 255.0)
			ptr->rgb.r = 255.0;
		if ((ptr->rgb.g *= intensity) >= 255.0)
			ptr->rgb.g = 255.0;
		if ((ptr->rgb.b *= intensity) >= 255.0)
			ptr->rgb.b = 255.0;

		if ((rgb.r = ptr->rgb.r * (1 - ptr->reflective) + rgb.r * ptr->reflective) > 255.0)
			rgb.r = 255.0;
		if ((rgb.g = ptr->rgb.g * (1 - ptr->reflective) + rgb.g * ptr->reflective) > 255.0)
			rgb.g = 255.0;
...
__kernel void mishania(__global char *image_data, __global t_scene *scene)
{
	...
	if (x == 320 && y == 700)
		flag = 1;
	put_point_to_image(image_data, x, y, cast_ray(&cl_scene, cl_scene.camera.center, pixel_pos_3d, flag));
